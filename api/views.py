...
class PlaylistWithSongUserDetailApiView(APIView):
    ...
    def get(self, request, id):
        playlist = self.get_object(id)
        serializer = PlaylistWithSongsUserSerializer(
            playlist, context={'user': request.user})
        return Response(serializer.data, status=status.HTTP_200_OK)

    def put(self, request, id):
        playlist = self.get_object(id)
        serializer = PlaylistWithSongsUserSerializer(
            playlist, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class AlbumDetailUserApiView(APIView):
    ...
    def get(self, request, id):
        album = self.get_object(id)
        serializer = Album4UserSerializer(
            album, context={'user': request.user})
        return Response(serializer.data, status=status.HTTP_200_OK)
...
class ArtistWithSongsApiView(APIView):
    ...
    def get(self, request, alias):
        artist = self.get_object(alias)
        # Lấy 10 bài hát có lượt nghe cao nhất của nghệ sĩ này
        songs = Song.objects.filter(artists=artist).order_by('-listen')[:10]
        serializer = ArtistWithSongsSerializer(artist)
        artist_data = serializer.data
        # Chuyển đổi các bài hát thành dữ liệu được serialize
        artist_data['songs'] = SongForArtistSerializer(songs, many=True).data
...
class Category7List(APIView):
    def get(self, request, format=None):
        categories = Category.objects.annotate(
            num_albums=Count('album')).filter(num_albums__gte=7)
...
class SearchAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        album_qs = Album.objects.filter(
            title__icontains=keyword).order_by('-like')[:9]
...
class SongFavorite(APIView):
    ...
    def put(self, request):
        ...
        print(user, song_id, islike, date.date(), duration)
        
        try:
            user_song = Usersong.objects.get(id_user=user, id_song=song_id)
            if islike is None:
                islike = user_song.islike
            serializer = UpdateUserSongSerializer(
                user_song, data={
                    'islike': islike,
                    'recenly_listen_date': date.date(),
                    'duration': duration
                }, partial=True)
            if serializer.is_valid():
                serializer.save()
...
class GetRandomAlbumFLWCate(APIView):
    def get(self, request):
        categories = Category.objects.all().order_by('?')[:1]
        category_id = categories[0].id
        albums = Album.objects.filter(cate=category_id)    
        serializer = AlbumSerializer(albums, many=True)
...
class GetSongListen(APIView):
    def get(self, request):
        ...
        songs = SongListenWeek.objects.filter(
            date_listen__gte=date.today() - timedelta(days=7)).order_by('-listen')[:30]
...
class UpdateSongListen(APIView):
    ...
    def post(self, request, id):
        ...
        song_in_week = SongListenWeek.objects.filter(
            song_id=id).order_by('-date_listen').first()
        if song_in_week:
            print(song_in_week)
            ...
            if song_in_week.date_listen > date_now - timedelta(days=7):
                ...
                SongListenWeek.objects.create(
                    song_id=song, date_listen=date_now, listen=1)
...
class GetPopularArtist(APIView):
    def get(self, request):
        artists = Artist.objects.all().order_by('-follow')[:20]
        serializer = ArtistSerializer(artists, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
class GetRecenlyListen(APIView):
    authentication_classes = [JWTAuthentication]
    permission_classes = [IsAuthenticated]
    def get(self, request):
        user = request.user.username
        ...
        if type == 'song':
            user_songs = Usersong.objects.filter(id_user=user, duration__gt=0).order_by('-recenly_listen_date')[:limit]
            songs = Song.objects.filter(id__in=[song.id_song for song in user_songs])

            serializer = SongForPlaylistSerializer(songs, many=True)
            return Response(serializer.data, status=status.HTTP_200_OK)
        else:
            # not found type
            return Response({'error': 'Not found type.'}, status=status.HTTP_400_BAD_REQUEST)
